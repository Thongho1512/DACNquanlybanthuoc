    ...
    public class CustomerPageService : ICustomerPageService
    {
        ...
        public async Task<ThuocDetailDto?> GetMedicineDetailAsync(int id)
        {
            ...
            if (thuoc == null || thuoc.TrangThai == false)
                throw new NotFoundException($"Kh�ng t�m th?y thu?c v?i id: {id}");
        ...
        public async Task<CustomerProfileDto?> GetCustomerProfileAsync(int customerId)
        {
            ...
            if (khachHang == null)
                throw new NotFoundException($"Kh�ng t�m th?y kh�ch h�ng v?i id: {customerId}");
        ...
        public async Task<DonHangDto?> GetOrderDetailAsync(int orderId)
        {
            ...
            if (donHang == null)
                throw new NotFoundException($"Kh�ng t�m th?y ??n h�ng v?i id: {orderId}");
        ...
        public async Task<ShipmentTrackingDto?> TrackShipmentAsync(int orderId)
        {
            ...
            if (donGiaoHang == null)
                throw new NotFoundException($"Kh�ng t�m th?y th�ng tin giao h�ng cho ??n h�ng: {orderId}");

            var lichSuCapNhat = new List<ShipmentHistoryDto>();
            if (donGiaoHang.NgayTao.HasValue)
                lichSuCapNhat.Add(new ShipmentHistoryDto { TrangThai = "PENDING", ThoiGian = donGiaoHang.NgayTao, MoTa = "??n h�ng v?a ???c t?o" });
            if (donGiaoHang.NgayXacNhan.HasValue)
                lichSuCapNhat.Add(new ShipmentHistoryDto { TrangThai = "CONFIRMED", ThoiGian = donGiaoHang.NgayXacNhan, MoTa = "??n h�ng ?� ???c x�c nh?n" });
            if (donGiaoHang.NgayLayHang.HasValue)
                lichSuCapNhat.Add(new ShipmentHistoryDto { TrangThai = "PICKING", ThoiGian = donGiaoHang.NgayLayHang, MoTa = "?ang chu?n b? h�ng" });
            if (donGiaoHang.NgayBatDauGiao.HasValue)
                lichSuCapNhat.Add(new ShipmentHistoryDto { TrangThai = "SHIPPING", ThoiGian = donGiaoHang.NgayBatDauGiao, MoTa = "?ang giao h�ng" });
            if (donGiaoHang.NgayGiaoThanhCong.HasValue)
                lichSuCapNhat.Add(new ShipmentHistoryDto { TrangThai = "DELIVERED", ThoiGian = donGiaoHang.NgayGiaoThanhCong, MoTa = "Giao h�ng th�nh c�ng" });
        ...
        public async Task<int?> GetStockByBranchAsync(int medicineId, int branchId)
        {
            ...
            if (thuoc == null)
                throw new NotFoundException($"Kh�ng t�m th?y thu?c v?i id: {medicineId}");
